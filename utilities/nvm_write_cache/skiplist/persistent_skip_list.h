...
namespace rocksdb{
        ...
        class persistent_SkipList {
            public:
            explicit
            persistent_SkipList(pool_base & pop, int32_t
            max_height = 12, int32_t
            branching_factor = 4);

            ~persistent_SkipList()
            {}

            void Insert(pool_base
            &pop,
            const char *key);
            ...
            persistent_ptr < Node > NewNode(pool_base & pop,
            const std::string
            &key,
            int height);
            ...
            KeyIsAfterNode(
            const std::string
            &key, persistent_ptr < Node > n) const;

            persistent_ptr < Node > FindGreaterOrEqual(
            const std::string
            &key) const;

            persistent_ptr < Node > FindLessThan(
            const std::string
            &key, persistent_ptr < persistent_ptr<Node>[] > prev = nullptr) const;
...
int main(int argc, char *argv[]) {
    ...
    if (rocksdb::file_exists(path.c_str()) != 0) {
        pop = pool<rocksdb::PersistentSkiplistWrapper>::create(path.c_str(), "layout", uint64_t(1024 * 1024 * 1024),
                                                               CREATE_MODE_RW);
    } else {
