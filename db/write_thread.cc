...
bool WriteThread::LinkOne(Writer* w, std::atomic<Writer*>* newest_writer) {
  ...
  while (true) {
    ...
    if (newest_writer->compare_exchange_weak(writers, w)) {	/* compare_exchange_weak()：CAS，等价于writers = w;(线程安全，原子操作) */
      return (writers == nullptr);			/* 若writers为nullptr，则说明当前writer是插入的第一个writer，那么当前writer为leader */
    }
  }
}
...
void WriteThread::JoinBatchGroup(Writer* w) {						/* 将一个Writer插入到WriteThread的Writer链表中 */
  ...
  if (linked_as_leader) {				/* LinkOne()返回true，则说明当前writer为GroupLeader */
    SetState(w, STATE_GROUP_LEADER);
  }

  TEST_SYNC_POINT_CALLBACK("WriteThread::JoinBatchGroup:Wait", w);

  if (!linked_as_leader) {				/* 否则，调用AwaitState等待状态改变 */
    /**
     * Wait util:
...
size_t WriteThread::EnterAsBatchGroupLeader(Writer* leader,
  ...
  Writer* w = leader;
  while (w != newest_writer) {			/* 从当前writer遍历到最新的writer */
    w = w->link_newer;
    ...
    w->write_group = write_group;			/* 选择符合条件的加入到write_group */
    size += batch_size;
