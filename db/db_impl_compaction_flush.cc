...
Status DBImpl::FlushMemTableToOutputFile(
  ...
  auto snapshot_checker = snapshot_checker_.get();	/* 猜测可能和snapshot相关，snapshot_checker类防止删除与快照相关的重复的key(flush/compaction过程) */
  ...
  if (s.ok()) {
    s = flush_job.Run(&logs_with_prep_tracker_, &file_meta);	/* 将immutable导盘时，需要预先赋好值的file_meta信息 */
  } else {
    flush_job.Cancel();
  }
  ...
  if (s.ok()) {
    InstallSuperVersionAndScheduleWork(cfd, &job_context->superversion_context,
                                       mutable_cf_options);		/* 1.Install SuperVersion; 2.Schedule Work */
    if (made_progress) {
      *made_progress = 1;
    }
  ...
  if (s.ok()) {
    ...
    NotifyOnFlushCompleted(cfd, &file_meta, mutable_cf_options,
                           job_context->job_id, flush_job.GetTableProperties());	/* 通知listeners flush操作完成(回调) */
    auto sfm = static_cast<SstFileManagerImpl*>(
        immutable_db_options_.sst_file_manager.get());
    if (sfm) {
      ...
      sfm->OnAddFile(file_path);			/* 添加一个new sst file: SstFileManagerImpl类内成员添加映射map[file_path] = file_size */
...
void DBImpl::NotifyOnFlushCompleted(ColumnFamilyData* cfd,
  ...
  bool triggered_writes_slowdown =
      (cfd->current()->storage_info()->NumLevelFiles(0) >=
       mutable_cf_options.level0_slowdown_writes_trigger);		/* Level 0 files数量过多，触发level0_slowdown_writes */
  bool triggered_writes_stop =
      (cfd->current()->storage_info()->NumLevelFiles(0) >=
       mutable_cf_options.level0_stop_writes_trigger);			/* 更严重，则可能导致level0_stop_writes */
  // release lock while notifying events
  mutex_.Unlock();
  {
    FlushJobInfo info;
...
Status DBImpl::FlushMemTable(ColumnFamilyData* cfd,
  ...
  uint64_t flush_memtable_id = 0;
  {
    ...
    cfd->imm()->FlushRequested();		/* 将imm_->flush_requested_置为true */
...
/*
 * Problem: 形参bg_flush_scheduled_, unscheduled_flushes_还在其他哪儿被修改值的
 */
void DBImpl::MaybeScheduleFlushOrCompaction() {
  mutex_.AssertHeld();
  if (!opened_successfully_) {									/* 打开数据库失败 */
    // Compaction may introduce data race to DB open
    return;
  }
  if (bg_work_paused_ > 0) {									/* 暂停后台工作 */
    // we paused the background work
    return;
  } else if (shutting_down_.load(std::memory_order_acquire)) {	/* 处于关闭数据库状态 */
  ...
  while (!is_flush_pool_empty && unscheduled_flushes_ > 0 &&
         bg_flush_scheduled_ < bg_job_limits.max_flushes) {		/* 先对unscheduled_flushes_的安排调度后台线程处理 */
    unscheduled_flushes_--;
    bg_flush_scheduled_++;
    env_->Schedule(&DBImpl::BGWorkFlush, this, Env::Priority::HIGH, this);	/* flush优先级设置为：HIGH */
  }

  // special case -- if high-pri (flush) thread pool is empty, then schedule
  // flushes in low-pri (compaction) thread pool.
  /* ???这里难道是多个immutable 在flush时做compaction??? */
  if (is_flush_pool_empty) {									/* 只有当flush线程池为空(执行完毕)，才会安排flush compaction线程 */
    while (unscheduled_flushes_ > 0 &&
           bg_flush_scheduled_ + bg_compaction_scheduled_ <
               bg_job_limits.max_flushes) {						/* 同unscheduled_flushes_ */
      unscheduled_flushes_--;
      bg_flush_scheduled_++;
      env_->Schedule(&DBImpl::BGWorkFlush, this, Env::Priority::LOW, this);	/* flush优先级设置为：LOW */
    }
  }
  ...
  while (bg_compaction_scheduled_ < bg_job_limits.max_compactions &&
         unscheduled_compactions_ > 0) {						/* 其他level进行compact的后台线程 */
...
void DBImpl::SchedulePendingFlush(ColumnFamilyData* cfd,
  ...
  if (!cfd->queued_for_flush() && cfd->imm()->IsFlushPending()) {
    AddToFlushQueue(cfd, flush_reason);							/* 将当前cfd加入到deque<ColumnFamilyData*> flush_queue_中 */
    ++unscheduled_flushes_;
  }
}
...
void DBImpl::SchedulePendingCompaction(ColumnFamilyData* cfd) {
  if (!cfd->queued_for_compaction() && cfd->NeedsCompaction()) {
    AddToCompactionQueue(cfd);									/* 将当前cfd加入到deque<ColumnFamilyData*> compaction_queue_中 */
    ++unscheduled_compactions_;
  }
}
...
void DBImpl::SchedulePendingPurge(std::string fname, std::string dir_to_sync,
  ...
  mutex_.AssertHeld();
  PurgeFileInfo file_info(fname, dir_to_sync, type, number, job_id);	/* 要被删除的file所拼凑的file_info信息 */
  /* move(): 接受一个参数并返回该参数的右值引用 */
  purge_queue_.push_back(std::move(file_info));					/* 将要被删除的file_info加入到deque<PurgeFileInfo> purge_queue_中 */
}
...
/*
 * @attention: 留意下FlushMemTableToOutputFile函数的参数各有啥作用
 */
Status DBImpl::BackgroundFlush(bool* made_progress, JobContext* job_context,
  ...
  while (!flush_queue_.empty()) {
    ...
    auto first_cfd = PopFirstFromFlushQueue();			/* 从flush_queue_中获取一个ColumnFamilyData* cfd */

    if (first_cfd->IsDropped() || !first_cfd->imm()->IsFlushPending()) {
      // can't flush this CF, try next one
      if (first_cfd->Unref()) {							/* 检查引用计数是否为1，若是则可以删除该cfd */
        delete first_cfd;
      }
      continue;
    }
  ...
  if (cfd != nullptr) {
    ...
    status = FlushMemTableToOutputFile(cfd, mutable_cf_options, made_progress,
                                       job_context, log_buffer);		/* 将memtable flush到磁盘的文件中(留意下函数调用的形参) */
    if (cfd->Unref()) {
      delete cfd;
    }
  }
...
void DBImpl::BackgroundCallFlush() {
                       ...
                       immutable_db_options_.info_log.get());
  {
    ...
    num_running_flushes_++;					/* 增加正在进行处理的flush计数 */

    auto pending_outputs_inserted_elem =
        CaptureCurrentFileNumberInPendingOutputs();

    Status s = BackgroundFlush(&made_progress, &job_context, &log_buffer);	/* 核心步骤：进行flush操作 */
    if (!s.ok() && !s.IsShutdownInProgress()) {
    ...
    FindObsoleteFiles(&job_context, !s.ok() && !s.IsShutdownInProgress());		/* flush处理失败，则删除所有创建的临时文件 */
    // delete unnecessary files if any, this is done outside the mutex
    if (job_context.HaveSomethingToClean() ||
        job_context.HaveSomethingToDelete() || !log_buffer.IsEmpty()) {
      mutex_.Unlock();
    ...
    num_running_flushes_--;				/* 无论flush成功还是失败，将正在运行的flush计数-1，同时被安排的flush计数-1 */
    bg_flush_scheduled_--;
    // See if there's more work to be done
    MaybeScheduleFlushOrCompaction();	/* 尝试触发下一次可能有的flush/compaction流程 */
    bg_cv_.SignalAll();
...
void DBImpl::BackgroundCallCompaction(PrepickedCompaction* prepicked_compaction,
                       ...
                       immutable_db_options_.info_log.get());
  {
    ...
    auto pending_outputs_inserted_elem =
        CaptureCurrentFileNumberInPendingOutputs();		/* 防止任何后台进程删除在当前点后创建的任意文件 */

    assert((bg_thread_pri == Env::Priority::BOTTOM &&
            bg_bottom_compaction_scheduled_) ||
           (bg_thread_pri == Env::Priority::LOW && bg_compaction_scheduled_));
    Status s = BackgroundCompaction(&made_progress, &job_context, &log_buffer,
                                    prepicked_compaction);		/* 执行后台compaction操作 */
...
if (!c) {
    ...
    for (const auto& f : *c->inputs(0)) {
      c->edit()->DeleteFile(c->level(), f->fd.GetNumber());			/* 删除compaction之前的input sst文件 */
    }
    status = versions_->LogAndApply(c->column_family_data(),
                                    *c->mutable_cf_options(), c->edit(),
                                    &mutex_, directories_.GetDbDir());	/* 更新新的version信息 */
    InstallSuperVersionAndScheduleWork(
        c->column_family_data(), &job_context->superversion_context,
        *c->mutable_cf_options(), FlushReason::kAutoCompaction);		/* 更新当前最新的SuperVersion */
    ROCKS_LOG_BUFFER(log_buffer, "[%s] Deleted %d files\n",
                     c->column_family_data()->GetName().c_str(),
                     c->num_input_files(0));
    *made_progress = true;												/* ???意味着compaction结束??? */
  } else if (!trivial_move_disallowed && c->IsTrivialMove()) {	/* 仅仅将sst文件往下层level移动 */
    ...
    for (unsigned int l = 0; l < c->num_input_levels(); l++) {
      ...
      for (size_t i = 0; i < c->num_input_files(l); i++) {		/* 将compaction_level层的sst文件移动到output_level层，仅修改元数据 */
...
//	compaction_job.RunKeyRange();
	...
	*made_progress = true;
    ...
    mutex_.Unlock();
    compaction_job.Run();								/* 调用CompactionJob::Run() */
...
void DBImpl::InstallSuperVersionAndScheduleWork(
  ...
  cfd->InstallSuperVersion(sv_context, &mutex_, mutable_cf_options);/* old_super_version准备push到vector<*>superversions_to_free中，更新新的super_version */
