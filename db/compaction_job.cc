...
Status CompactionJob::Run() {
  ...
  for (auto& thread : thread_pool) {			/* 等待其它线程执行完毕 */
    thread.join();
  }
  ...
  if (status.ok()) {							/* 每个线程均成功执行结束 */
    thread_pool.clear();
    std::vector<const FileMetaData*> files_meta;			/* 这里保存每个sub_compaction thread完毕时对应的中间阶段性output_file */
    for (const auto& state : compact_->sub_compact_states) {
    ...
    auto verify_table = [&](Status& output_status) {		/* ???lambda表达式??? */
      while (true) {
        size_t file_idx = next_file_meta_idx.fetch_add(1);	/* 从0开始递增 */
        if (file_idx >= files_meta.size()) {				/* 第n-1个file后结束 */
          break;
        }
...
Status CompactionJob::OpenCompactionOutputFile(
    SubcompactionState* sub_compact) {									/* 在设定目录下创建output file，包括生成新的file_number */
  assert(sub_compact != nullptr);
  assert(sub_compact->builder == nullptr);
