...
class WriteThread {
 public:
  ...
  struct WriteGroup {
    ...
    Status status;
    std::atomic<size_t> running;		/* 貌似记录write_group保存链表的writer节点数量(size) */
    size_t size = 0;
  ...
  struct Writer {								/* 表示一次写操作，通常完成两个步骤：写WAL；写Memtable */
    WriteBatch* batch;
    bool sync;
    bool no_slowdown;
    bool disable_wal;
    ...
    uint64_t log_used;  // log number that this batch was inserted into：该batch需要插入的WAL文件对应的log_number
    uint64_t log_ref;   // log number that memtable insert should reference：当前要插入到的memtable对应WAL文件的log_number
    WriteCallback* callback;
    bool made_waitable;          // records lazy construction of mutex and cv
    std::atomic<uint8_t> state;  // write under StateMutex() or pre-link
    WriteGroup* write_group;
    SequenceNumber sequence;  // the sequence number to use for the first key：write操作的首个key的sequence_number
 ...
 private:
  ...
  const bool allow_concurrent_memtable_write_;				/* 是否允许memtable并发写操作 */

  // Enable pipelined write to WAL and memtable.			/* 是否允许WAL和memtbale的pipeline写 */
