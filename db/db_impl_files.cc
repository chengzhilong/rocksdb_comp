...
void DBImpl::FindObsoleteFiles(JobContext* job_context, bool force,
  ...
  if (!alive_log_files_.empty() && !logs_.empty()) {
    ...
    while (alive_log_files_.begin()->number < min_log_number) {		/* 遍历所有活着的WAL，找出log_number < min_log_number的文件 */
      ...
      if (immutable_db_options_.recycle_log_file_num >
        ...
        job_context->log_delete_files.push_back(earliest.number);	/* 将其加入到vector<uint64_t> log_delete_files里面 */
      }
    ...
    while (!logs_.empty() && logs_.front().number < min_log_number) {	/* 遍历所有WAL文件 */
