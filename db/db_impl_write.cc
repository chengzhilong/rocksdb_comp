...
Status DBImpl::WriteImpl(const WriteOptions& write_options,
  ...
  last_batch_group_size_ =
      write_thread_.EnterAsBatchGroupLeader(&w, &write_group);		/* 此时w是STATE_GROUP_LEADER，需要建立自己的write_group */
...
Status DBImpl::PipelinedWriteImpl(const WriteOptions& write_options,
  ...
  /* 返回的若当前Writer对象是leader，则将此leader下的所有writer都链接到一个WriteGroup中,并开始写入WAL */
...
Status DBImpl::PreprocessWrite(const WriteOptions& write_options,
  ...
  if (UNLIKELY(status.ok() && write_buffer_manager_->ShouldFlush())) {
    ...
    status = HandleWriteBufferFull(write_context);			/* 调用函数 MaybeScheduleFlushOrCompaction() */
  }
...
Status DBImpl::WriteToWAL(const WriteThread::WriteGroup& write_group,
  ...
  WriteBatchInternal::SetSequence(merged_batch, sequence);			/* 将sequence作为merged_batch的首个sequence_number */
...
Status DBImpl::SwitchWAL(WriteContext* write_context) {
  ...
  for (auto cfd : *versions_->GetColumnFamilySet()) {
    ...
    if (cfd->OldestLogToKeep() <= oldest_alive_log) {	/* 意味着cfd指向的memtable还有部分数据在WAL中没有记录 */
...
Status DBImpl::HandleWriteBufferFull(WriteContext* write_context) {
  ...
  if (cfd_picked != nullptr) {
    ...
    if (status.ok()) {
      cfd_picked->imm()->FlushRequested();					/* 将imm_->flush_requested_置为true */
...
// REQUIRES: mutex_ is held
// REQUIRES: this thread is currently at the front of the writer queue
/* 切换Memtable，在做flush之前切换(生成新的memtable，然后把旧的memtable刷新到磁盘) */
Status DBImpl::SwitchMemtable(ColumnFamilyData* cfd, WriteContext* context,
  ...
  mutex_.Unlock();
  {
    if (creating_new_log) {
      ...
      if (recycle_log_number) {					/* 若immutable_db_options_设置了需要重用旧的log_number */
        ROCKS_LOG_INFO(immutable_db_options_.info_log,
