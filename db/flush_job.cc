...
/* 获取需要进行flush的memtable，一个CF可能包含多个待flush的memtable，这里可能选多个memtable */
void FlushJob::PickMemTable() {
  db_mutex_->AssertHeld();
  assert(!pick_memtable_called);
  pick_memtable_called = true;
  // Save the contents of the earliest memtable as a new Table
  cfd_->imm()->PickMemtablesToFlush(&mems_);	/* 获取一个memtable列表 */
  if (mems_.empty()) {
    return;
  }
  ...
  // this flush.
  MemTable* m = mems_[0];						/* mems_列表根据create_time升序排列 */
  edit_ = m->GetEdits();
  ...
  meta_.fd = FileDescriptor(versions_->NewFileNumber(), 0, 0);	/* 这里先确定好flush到磁盘上的第一个sst文件的元数据 */
...
Status FlushJob::Run(LogsWithPrepTracker* prep_tracker,
  ...
  if (s.ok() && file_meta != nullptr) {
    *file_meta = meta_;			/* 此处meta_即FlushJob::PickMemTable()函数被赋值的meta_(每次flush到L0层只会生成一个sst文件) */
  }
  RecordFlushIOStats();
...
Status FlushJob::WriteLevel0Table() {
  ...
  db_mutex_->AssertHeld();
  const uint64_t start_micros = db_options_.env->NowMicros();				/* 记录start_time */
  Status s;
  {
    ...
    for (MemTable* m : mems_) {											/* 这里mems_可能有多个immutable */
      ROCKS_LOG_INFO(
          db_options_.info_log,
          "[%s] [JOB %d] Flushing memtable with next log file: %" PRIu64 "\n",
          cfd_->GetName().c_str(), job_context_->job_id, m->GetNextLogNumber());
      memtables.push_back(m->NewIterator(ro, &arena));					/* 将要flush的memtable的迭代器传递给memtables */
      auto* range_del_iter = m->NewRangeTombstoneIterator(ro);
      if (range_del_iter != nullptr) {
        range_del_iters.push_back(range_del_iter);						/* 同时，这些memtable也意味着要被删除，先记录下来 */
      }
...

    {
      ScopedArenaIterator iter(
          NewMergingIterator(&cfd_->internal_comparator(), &memtables[0],
                             static_cast<int>(memtables.size()), &arena));	/* 这里先创建两个迭代器，iter和range_del_iter */
      ...
      s = BuildTable(
          ...
          oldest_key_time, write_hint);											/* 生成SSTable文件的所有工作都交给BuildTable() */
